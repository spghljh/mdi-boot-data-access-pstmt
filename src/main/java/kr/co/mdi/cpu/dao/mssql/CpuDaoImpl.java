...
public class CpuDaoImpl extends AbstractJdbcDao implements CpuDao {
    ...
    public CpuDTO insertCpu(CpuDTO cpu) {
        // MSSQL은 IDENTITY 컬럼을 사용 → SCOPE_IDENTITY()로 PK 조회
        String sql = "INSERT INTO cpu (name_cpu, release_cpu, core_cpu, thread_cpu, maxghz_cpu, minghz_cpu, type_code_cpu, manf_code_cpu) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
