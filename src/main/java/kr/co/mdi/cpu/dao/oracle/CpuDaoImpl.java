...
public class CpuDaoImpl extends AbstractJdbcDao implements CpuDao {
    ...
    public CpuDTO insertCpu(CpuDTO cpu) {
        // Oracle은 시퀀스 사용
        String sql = "INSERT INTO cpu (id_cpu, name_cpu, release_cpu, core_cpu, thread_cpu, maxghz_cpu, minghz_cpu, type_code_cpu, manf_code_cpu) " +
                     "VALUES (cpu_seq.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql, new String[]{"id_cpu"})) {
