...
public class DeviceDaoImpl extends AbstractJdbcDao implements DeviceDao {
    ...
    public DeviceDTO insertDevice(DeviceDTO device) {
        // MSSQL은 IDENTITY 컬럼을 사용 → RETURN_GENERATED_KEYS로 PK 조회
        String sql = "INSERT INTO device (name_device, id_cpu, lineup_device, release_device, weight_device, type_code_device, manf_code_device) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
