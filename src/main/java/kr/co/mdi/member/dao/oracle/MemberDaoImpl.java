...
public class MemberDaoImpl extends AbstractJdbcDao implements MemberDao {
    ...
    public MemberDTO insertMember(MemberDTO member) {
        // Oracle은 시퀀스 사용 → member_seq.NEXTVAL
        String sql = "INSERT INTO member (id_member, id, pass, name, email, role, status, email_verified) VALUES (member_seq.NEXTVAL, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql, new String[]{"id_member"})) {
