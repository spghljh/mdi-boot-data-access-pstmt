...
public class MemberDaoImpl extends AbstractJdbcDao implements MemberDao {
    ...
    public MemberDTO insertMember(MemberDTO member) {
        // PostgreSQL은 RETURNING 구문 사용
        String sql = "INSERT INTO member (id, pass, name, email, role, status, email_verified) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id_member";
        try (Connection conn = getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    member.setIdMember(rs.getLong("id_member"));
                }
            }
            return member;
        } catch (Exception e) {
